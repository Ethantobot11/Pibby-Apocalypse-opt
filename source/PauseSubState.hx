...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuItemsOG:Array<String> = ['Resume', 'Restart Song', 'Options', 'Toggle Botplay', 'Chart Editor', 'Exit to menu'];
	...
	public function new(x:Float, y:Float)
	{
		...
		FlxTween.tween(bg, {alpha: 0.6}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.001});
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted && (cantUnpause <= 0 || !ClientPrefs.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				FlxTween.tween(PlayState.instance.vocals, {pitch: 0.001}, 6, {onComplete: e -> PlayState.instance.vocals.stop()});
				FlxG.sound.music.fadeOut(6.5);
				PlayState.instance.vocals.fadeOut(6.5);
				FlxG.camera.fade(FlxColor.BLACK, 6, false, () -> {
					FlxG.sound.music.stop();
					new FlxTimer().start(2, e -> PlayState.instance.youCheatedRah());
				});
				case "Exit to menu":
